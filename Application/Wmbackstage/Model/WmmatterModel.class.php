...
class WmmatterModel extends Model {
     ...
     * @return mixed
     */
    public function get_one_matter_by_id($matter_id = 0) {
        if(!$matter_id)
            throw_exception('素材ID号不存在！');
        return $this->_db->where('matter_id = ' . $matter_id)->find();
    }

    public function update_one_matter_by_id($matter_id = 0,$data = array()) {
        if(!$matter_id)
            throw_exception('素材ID号不存在！');
        if(!$data || !isset($data))
            throw_exception('数据不存在！');
        return $this->_db->where('matter_id = ' . $matter_id)->save($data);
    }

    public function delete_one_matter_by_id($matter_id = 0) {
        if(!$matter_id)
            throw_exception('素材ID号不存在！');
        return $this->_db->where('matter_id = ' . $matter_id)->delete();
    }
}