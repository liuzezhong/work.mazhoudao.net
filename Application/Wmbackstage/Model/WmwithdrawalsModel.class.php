...
class WmwithdrawalsModel extends Model {
    ...
    public function get_all_withdrawals_limit($data = array(),$page,$pageSize = 10) {
        ...
        if(isset($data['tixian_type'])) {
            $selectData['tixian_type'] = $data['tixian_type'];
        }
    ...
    /**
     *功能：判断时间段是否有周末，有两个已知时间戳，判断该时间戳内是否包含周六周天。     *
     * @param begin 开始时间
     * @param last 结束时间
     */
     ...
     * @return mixed
     */
    public function get_all_withdrawals($data = array()) {
        ...
        if(isset($data['begin_date']) && isset($data['end_date'])) {
            $data['sql_date'] = array($data['begin_date'],$data['end_date']);
            $selectData['pay_time'] = array('BETWEEN',$data['sql_date']);
        }
    ...
    public function delete_wmwithdrawals_by_id($data = array()) {
        $selectData['serial_number'] = array('IN',$data);
        $test =  $this->_db->where($selectData)->delete();
        return $test;
    }

    public function find_wmwithdrawals_by_userid($user_id = 0) {
        return $this->_db->where('user_id = ' . $user_id)->select();
    }

    public function update_fapiao_status($serial_number = 0, $data = array()) {
